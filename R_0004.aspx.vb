...
Partial Class R_0004
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Btn_Articulo_Final.Attributes.Add("onclick", "window.open('Bus_Cat.aspx?Catalogo=ARTICULO&Num=2',null,'left=400, top=100, height=450, width= 800, status=no, resizable=no, scrollbars=no, toolbar=no,location= no, menubar=no');")
    ...
    Protected Sub Ima_Imprimir_Click(sender As Object, e As System.EventArgs) Handles Ima_Imprimir.Click
        ...
        'Script = "<script type='text/javascript'>window.open('Visor_Reporte.aspx?Reporte=R_0004&Elem_I="
        ...
        'Script &= T_Articulo_Inicial.Text.Trim & "&Art_F=" & T_Articulo_Final.Text.Trim & "&Ref=" & T_Referencia.Text.Trim & "&Cl="
        'Script &= T_Clasificacion.Text.Trim & "');</script>"
    ...
    Protected Sub Btn_Acepta_Imp_Click(sender As Object, e As System.EventArgs) Handles Btn_Acepta_Imp.Click
        ...
        Dim Script As String = ""
        Script = "<script type='text/javascript'>window.open('Visor_Reporte.aspx?" & Impresora & "Reporte=R_0004&Elem_I="
        Script &= T_Elemento_Inicial.Text.Trim & "&Elem_F=" & T_Elemento_Final.Text.Trim & "&Act=" & T_Actividad.Text.Trim & "&TpCos="
        Script &= T_Tipo_Costo.Text.Trim & "&Fecha_Inicio=" & T_Fecha_Inicial.Text.Trim & "&Fecha_Fin=" & T_Fecha_Final.Text.Trim & "&Art_I="
        Script &= T_Articulo_Inicial.Text.Trim & "&Art_F=" & T_Articulo_Final.Text.Trim & "&Ref=" & T_Referencia.Text.Trim & "&Cl="
        Script &= T_Clasificacion.Text.Trim & "');</script>"
        ...
        Try
            ...
            Select Case Tipo_Catalogo
                ...
                Case Else
                    ...
                    Btn_Elemento_Final.Attributes.Add("onclick", "")
            End Select
            G.Tsql = "Select * from Tipo_Salida where Tipo=" & Val(T_Tipo_Costo.Text)
            G.com.CommandText = G.Tsql
            G.dr = G.com.ExecuteReader
            If G.dr.Read Then
                If G.dr("Referencia") = "S" Then
                    T_Referencia.Enabled = True
                    T_Ref_Descripcion.Enabled = True
                    Btn_Referencia.Attributes.Add("onclick", "window.open('Bus_Cat.aspx?Catalogo=REFERENCIA',null,'left=400, top=100, height=450, width= 800, status=no, resizable=no, scrollbars=no, toolbar=no,location= no, menubar=no');")
                    Btn_Referencia.Attributes.Add("style", "cursor:pointer;")
                    T_Referencia.Attributes.Add("onkeypress", "javascript: ValidaSoloNumeros('" & T_Referencia.ClientID & "');")
                Else
                    T_Referencia.Enabled = False
                    T_Ref_Descripcion.Enabled = False
                    Btn_Referencia.Attributes.Add("onclick", "")
                    Btn_Referencia.Attributes.Add("style", "cursor:not-allowed;")
                End If

                If Val(T_Tipo_Costo.Text) <> 19 Then
                    Btn_Clasificacion.Attributes.Add("onclick", "window.open('Bus_Cat.aspx?Catalogo=GASTO" & "',null,'left=400, top=100, height=450, width= 800, status=no, resizable=no, scrollbars=no, toolbar=no,location= no, menubar=no');")
                    Btn_Clasificacion.Attributes.Add("style", "cursor:pointer;")

                Else
                    Btn_Clasificacion.Attributes.Add("onclick", "window.open('Bus_Cat.aspx?Catalogo=GASTO_19" & "',null,'left=400, top=100, height=450, width= 800, status=no, resizable=no, scrollbars=no, toolbar=no,location= no, menubar=no');")
                    Btn_Clasificacion.Attributes.Add("style", "cursor:pointer;")

                End If
                If G.dr("Clasifica") = "S" Then
                    T_Clasificacion.Enabled = True
                    T_Clas_Descripcion.Enabled = True
                    T_Clasificacion.Attributes.Add("onkeypress", "javascript: ValidaSoloNumeros('" & T_Clasificacion.ClientID & "');")
                    Btn_Clasificacion.Attributes.Add("style", "cursor:pointer;")
                Else
                    T_Clasificacion.Enabled = False
                    T_Clas_Descripcion.Enabled = False
                    Btn_Clasificacion.Attributes.Add("onclick", "")
                    Btn_Clasificacion.Attributes.Add("style", "cursor:not-allowed;")
                End If

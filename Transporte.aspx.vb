...
Partial Class Transporte
    ...
    Private Function validar() As Boolean
        ...
        If T_Numero.Text.Trim = "" Then
            Msg_Error("Numero inválida") : Exit Function
        End If
        Return True
    End Function
    ...
    Private Function LLena_Datatable() As DataTable
        ...
        Try
            G.cn.Open()
            Session("dt") = New DataTable
            G.Tsql = "Select Numero,Descripcion from Transporte where Compañia = " & G.Empresa_Numero & " and Sucursal = " & G.Obra
    ...
    Protected Sub Ima_Guarda_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Ima_Guarda.Click
        ...
        Try
            ...
            If Movimiento.Value = "Alta" Then
                T_Numero.Text = Siguiente()
                G.cn.Open()
                Tsql = "Select Descripcion from Transporte where Descripcion=" & Pone_Apos(T_Nombre.Text)
                G.com.CommandText = Tsql
                If Not G.com.ExecuteScalar Is Nothing Then
                    Msg_Error("Ya existe una Transporte con esa Descripción") : Exit Sub
                End If
                G.Tsql = "Insert into Transporte (Numero,Descripcion,Baja,Fec_cambio,Compañia,Sucursal) values ("
                G.Tsql &= T_Numero.Text.Trim
                G.Tsql &= "," & Pone_Apos(T_Nombre.Text.Trim)
                G.Tsql &= "," & "''"
                G.Tsql &= "," & Pone_Apos(Fecha_AMD(DateTime.Now().ToShortDateString()))
                G.Tsql &= "," & G.Empresa_Numero
                G.Tsql &= "," & G.Obra & ")"
                G.com.CommandText = G.Tsql
                G.com.ExecuteNonQuery()
                AñadeFilaGrid(T_Numero.Text.Trim, T_Nombre.Text.Trim)
                LimpiaCampos()
            End If
            If Movimiento.Value = "Cambio" Then
                G.cn.Open()
                G.Tsql = "Update Transporte set Descripcion = " & Pone_Apos(T_Nombre.Text.Trim)
                ...
                G.Tsql &= " and Compañia =" & G.Empresa_Numero
                G.Tsql &= " and Sucursal =" & G.Obra
                G.com.CommandText = G.Tsql
                G.com.ExecuteNonQuery()
                If Ch_Baja.Checked = True Then
                    EliminaFilaGrid(T_Numero.Text.Trim)
                End If
                CambiaFilaGrid(T_Numero.Text.Trim, T_Nombre.Text.Trim)
                LimpiaCampos()
            End If
            If Movimiento.Value = "Baja" Then
                G.cn.Open()
                G.Tsql = "Update Transporte set Baja = '*' ,"
                G.Tsql &= " Fec_cambio = " & Pone_Apos(Fecha_AMD(DateTime.Now().ToShortDateString()))
                G.Tsql &= " Where Numero = " & Val(T_Numero.Text.Trim)
                G.Tsql &= " and Compañia = " & G.Empresa_Numero
                G.Tsql &= " and Sucursal = " & G.Obra
    ...
    Private Function Siguiente() As Integer
        ...
        Try
            G.cn.Open()
            G.Tsql = "Select Max(Numero) from Transporte"
            G.com.CommandText = G.Tsql

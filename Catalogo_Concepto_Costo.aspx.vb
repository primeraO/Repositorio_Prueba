...
Partial Class Catalogo_Concepto_Costo
    ...
    Private Sub CrearCamposTabla()
        ...
        Session("dt").Columns.Add("Concepto", Type.GetType("System.Int64")) : Session("dt").Columns("Concepto").DefaultValue = 0
        ...
        clave(0) = Session("dt").Columns("Concepto")
    ...
    Private Function LLena_Datatable() As DataTable
        ...
        Try
            ...
            G.cn.Open()
            G.Tsql = "Select Concepto,Descripcion from Concepto_Costo "
    ...
    Private Function Siguiente() As Integer
        ...
        Try
            G.cn.Open()
            G.Tsql = "Select Max(Concepto) from Concepto_Costo"
            G.Tsql &= " Where Cia=" & Session("Cia")
            G.Tsql &= " and Obra=" & Pone_Apos(Session("Obra"))
            G.com.CommandText = G.Tsql
    ...
    Protected Sub Ima_Guarda_Click(sender As Object, e As System.EventArgs) Handles Ima_Guarda.Click
        ...
        Try
            ...
            If Movimiento.Value = "Alta" Then
                T_Numero.Text = Siguiente()
                G.cn.Open()
                Tsql = "Select Descripcion from Concepto_Costo where Descripcion=" & Pone_Apos(T_Descripcion.Text)
                G.com.CommandText = Tsql
                If Not G.com.ExecuteScalar Is Nothing Then
                    Msg_Error("Ya existe el Nombre del Concepto") : Exit Sub
                End If
                G.Tsql = "Insert into Concepto_Costo (Cia,Obra,Concepto,Descripcion,Fecha_Seg,Hora_Seg,Baja) values ("
            ...
            If Movimiento.Value = "Cambio" Then
                G.cn.Open()
                G.Tsql = "Update Concepto_Costo set Cia=" & Val(Session("Cia"))
                ...
                G.Tsql &= " Where Cia=" & Session("Cia")
                G.Tsql &= " and Obra=" & Pone_Apos(Session("Obra"))
                G.Tsql &= " and Concepto=" & Val(T_Numero.Text.Trim)
                G.com.CommandText = G.Tsql
                G.com.ExecuteNonQuery()
    ...
    Protected Sub GridView1_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles GridView1.SelectedIndexChanged
        Dim clave(0) As String
        clave(0) = GridView1.DataKeys(GridView1.SelectedIndex).Item("Concepto").ToString
        Dim f As DataRow = Session("dt").Rows.Find(clave)
        If Not f Is Nothing Then
            T_Numero.Text = f.Item("Concepto")
            T_Descripcion.Text = f.Item("Descripcion")
